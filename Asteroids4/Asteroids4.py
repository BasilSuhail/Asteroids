...
class Player(pygame.sprite.Sprite):
    def __init__(self):
        super().__init__()
        self.base_image = pygame.Surface((36, 24), pygame.SRCALPHA)
        pygame.draw.polygon(self.base_image, WHITE, [(36, 12), (0, 0), (0, 24)])
    ...
    def update(self, *args):
        ...
        self.original_image = pygame.transform.rotate(self.base_image, angle)
    ...
    def activate_shockwave(self):
        ...
        if now - self.last_shockwave_time > self.shockwave_cooldown:
            self.last_shockwave_time = now
            # MODIFIED: Create a single Shockwave object
            return Shockwave(self.pos)
        return None 
...
# REMOVED: ShockwaveParticle class

# NEW: Shockwave class
class Shockwave(pygame.sprite.Sprite):
    def __init__(self, center_pos):
        super().__init__()
        self.center_pos = pygame.math.Vector2(center_pos)
        self.start_radius = 10
        self.max_radius = 200
        self.lifespan = 25 # Frames the shockwave will last
        self.current_frame = 0

        self.image = pygame.Surface((1, 1), pygame.SRCALPHA) # Dummy surface initially
        self.rect = self.image.get_rect(center=self.center_pos)
        self.mask = pygame.mask.from_surface(self.image) # Will be updated each frame

    def update(self, *args):
        self.current_frame += 1
        if self.current_frame > self.lifespan:
            self.kill()
            return

        progress = self.current_frame / self.lifespan
        current_radius = int(self.start_radius + (self.max_radius - self.start_radius) * progress)
        alpha = int(255 * (1 - progress)) # Fade out

        # Create a new surface for the current frame
        surface_size = current_radius * 2
        self.image = pygame.Surface((surface_size, surface_size), pygame.SRCALPHA)
        pygame.draw.circle(self.image, (*SHOCKWAVE_COLOR, alpha), (current_radius, current_radius), current_radius, 2) # Draw a hollow circle
        
        self.rect = self.image.get_rect(center=self.center_pos)
        self.mask = pygame.mask.from_surface(self.image) # Update mask for collisions
    ...
    def update(self, player_pos):
        ...
        if self.state == 'entering':
            ...
            if now - self.last_shot_time > self.fire_rate:
                ...
                new_bullet = EnemyBullet(self.rect.center, player_pos)
...
class EnemyShip(pygame.sprite.Sprite):
    def __init__(self, target):
        ...
        self.strafe_timer = 0
        self.strafe_duration = 3000
        self.strafe_direction = pygame.math.Vector2(1, 0)
    ...
    def update(self, player_pos):
        ...
        if self.state == 'approaching':
            if distance > 250:
                ...
                self.state = 'strafing'
                self.strafe_timer = pygame.time.get_ticks()
            ...
            if now - self.last_shot_time > self.fire_rate:
                self.last_shot_time = now
                new_bullet = EnemyBullet(self.pos, player_pos)
...
def draw_text(text, x, y, font_obj=small_font, color=WHITE):
    text_surface = font_obj.render(text, True, color)
...
def main():
    ...
    # MODIFIED: Renamed to shockwaves, will contain one object at a time
    ...
    while running:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.button == 3 and not game_over:
                    new_shockwave = player.activate_shockwave() # MODIFIED
                    if new_shockwave: # MODIFIED
                        all_sprites.add(new_shockwave) # MODIFIED
                        shockwaves.add(new_shockwave) # MODIFIED
        ...
        if not game_over:
            ...
            # MODIFIED: Shockwave collision logic
            for shockwave in shockwaves:
                # Collide shockwave with asteroids
                shockwave_hits_asteroids = pygame.sprite.spritecollide(shockwave, asteroids, False, pygame.sprite.collide_mask)
                for hit_asteroid in shockwave_hits_asteroids:
                    # Damage the asteroid only once per shockwave
                    if hit_asteroid not in getattr(shockwave, 'hit_targets', []):
                        ...
                        if hit_asteroid.health <= 0:
                            ...
                            if hit_asteroid.size > 1:
                                for _ in range(2):
                                    ...
                                    all_sprites.add(new_asteroid); asteroids.add(new_asteroid)
                            hit_asteroid.kill()
                        # Keep track of hit targets to prevent multiple hits from one shockwave frame
                        if not hasattr(shockwave, 'hit_targets'): shockwave.hit_targets = []
                        shockwave.hit_targets.append(hit_asteroid)

                # Collide shockwave with enemy ships
                shockwave_hits_ships = pygame.sprite.spritecollide(shockwave, enemy_ships, False, pygame.sprite.collide_mask)
                for hit_ship in shockwave_hits_ships:
                    if hit_ship not in getattr(shockwave, 'hit_targets', []):
                        hit_ship.health -= 3 # A single shockwave should significantly damage or destroy a ship
                        if hit_ship.health <= 0:
                            score += 50; all_sprites.add(Explosion(hit_ship.rect.center, 'medium')); hit_ship.kill()
                        if not hasattr(shockwave, 'hit_targets'): shockwave.hit_targets = []
                        shockwave.hit_targets.append(hit_ship)
            # END MODIFICATION
        ...
        if cooldown_elapsed < player.shockwave_cooldown:
            ...
            draw_text_center("Shockwave", bar_x + bar_total_width / 2, bar_y + bar_height / 2, small_font) # Use small_font here
        else:
            pygame.draw.rect(screen, UI_READY_COLOR, (bar_x + 1, bar_y + 1, bar_total_width - 2, bar_height - 2))
            draw_text_center("SHOCKWAVE READY", bar_x + bar_total_width / 2, bar_y + bar_height / 2, small_font, BLACK) # Use small_font here and black text for visibility
        ...
        if game_over:
            draw_text_center("GAME OVER", SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2 - 20)
            draw_text_center("Press 'R' to Restart", SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2 + 20)
