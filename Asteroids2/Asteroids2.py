...
# --- NOTE: All external asset loading has been removed ---
...
class Player(pygame.sprite.Sprite):
    def __init__(self):
        ...
        # --- MODIFIED: Shockwave cooldown reduced to 1 second ---
    ...
    def activate_shockwave(self):
        ...
        if now - self.last_shockwave_time > self.shockwave_cooldown:
            self.last_shockwave_time = now; return ShockwaveParticle(self.pos)
        return None 
...
class ShockwaveParticle(pygame.sprite.Sprite):
    def __init__(self, pos):
        super().__init__()
...
def main():
    ...
    enemy_ships = pygame.sprite.Group() # --- NEW ---
    player = Player()
    ...
    # --- NEW: Enemy ship spawn timer ---
    enemy_ship_spawn_interval = random.randint(5000, 10000)
    last_enemy_ship_spawn = pygame.time.get_ticks()
    # --- END NEW ---
    ...
    while running:
        ...
        if not star_enemy_group:
            if now - last_spawn_time > spawn_interval and not game_over:
                last_spawn_time = now
                target = player.pos if random.random() < 0.5 else None
                new_asteroid = Asteroid(3, target_pos=target)
                all_sprites.add(new_asteroid); asteroids.add(new_asteroid)

        if now - last_star_spawn_time > star_spawn_interval and not star_enemy_group and not game_over:
            last_star_spawn_time = now
            star_spawn_interval = random.randint(45000, 60000)
            star = StarEnemy()
            all_sprites.add(star); star_enemy_group.add(star)
            
            for ast in asteroids:
                all_sprites.add(Explosion(ast.rect.center, 'small')); ast.kill()
        
        # --- NEW: Enemy ship spawning ---
        if now - last_enemy_ship_spawn > enemy_ship_spawn_interval and not star_enemy_group and not game_over:
            last_enemy_ship_spawn = now
            enemy_ship_spawn_interval = random.randint(5000, 15000)
            
            num_to_spawn = random.choice([1, 2, 2, 3])
            for _ in range(num_to_spawn):
                ship = EnemyShip(player)
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT: running = False
                ...
                if event.button == 3 and not game_over:
                    ...
                    if new_shockwave: all_sprites.add(new_shockwave); shockwave_group.add(new_shockwave)
            elif event.type == pygame.KEYDOWN:
                if event.key == pygame.K_r and game_over: main()
        ...
        if not game_over:
            ...
            for ship in hits_on_ships:
                ...
                if ship.health <= 0:
                    score += 50
                    all_sprites.add(Explosion(ship.rect.center, 'medium'))
                    ship.kill()
            ...
            if shockwave_group:
                for shockwave in shockwave_group:
                    ...
                    sw_hits_star = pygame.sprite.spritecollide(shockwave, star_enemy_group, False, pygame.sprite.collide_mask)
                    for star in sw_hits_star:
                        if star not in shockwave.hit_targets:
                            star.health -= 10; star.hit_timer = 5
                            if star.health <= 0:
                                score += 500;
                                for _ in range(50): all_sprites.add(Debris(star.rect.center))
                                all_sprites.add(Explosion(star.rect.center, 'large')); all_sprites.add(Explosion(star.rect.center, 'large'))
                                star.kill()
                            shockwave.hit_targets.add(star)
            ...
            player_asteroid_hits = pygame.sprite.spritecollide(player, asteroids, False, pygame.sprite.collide_mask)
            if player_asteroid_hits and not player.invincible:
                life_was_lost = False
                for hit_asteroid in player_asteroid_hits:
                    if player.take_damage(25): life_was_lost = True
                    hit_asteroid.health -= 3; hit_asteroid.hit_timer = 5
                    if hit_asteroid.health <= 0:
                        score += 10
                        for _ in range(random.randint(5, 10)): all_sprites.add(Debris(hit_asteroid.rect.center))
            ...
            player_star_hits = pygame.sprite.spritecollide(player, star_enemy_group, False, pygame.sprite.collide_mask)
            if player_star_hits:
            ...
            player_hit_by_bullet = pygame.sprite.spritecollide(player, enemy_bullets, True, pygame.sprite.collide_mask)
        ...
        bar_x = SCREEN_WIDTH - 150; bar_y = SCREEN_HEIGHT - 30; bar_total_width = 120; bar_height = 20
        ...
        if game_over:
            draw_text("GAME OVER", SCREEN_WIDTH / 2 - 70, SCREEN_HEIGHT / 2 - 20)
            draw_text("Press 'R' to Restart", SCREEN_WIDTH / 2 - 100, SCREEN_HEIGHT / 2 + 20)
