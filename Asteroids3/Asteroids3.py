...
# --- MODIFIED: Fonts are now defined globally ---
    ...
    def activate_shockwave(self):
        ...
        if now - self.last_shockwave_time > self.shockwave_cooldown:
            ...
            for i in range(num_particles):
                ...
                particles.append(ShockwaveParticle(self.pos, angle))
            return particles
        return None 
...
class ShockwaveParticle(pygame.sprite.Sprite):
    def __init__(self, pos, angle):
        super().__init__()
        self.pos = pygame.math.Vector2(pos)
        self.vel = pygame.math.Vector2(1, 0).rotate(angle) * random.uniform(5, 8)
        self.max_lifespan = 25
        self.lifespan = self.max_lifespan
        
        self.original_image = pygame.Surface((12, 3), pygame.SRCALPHA)
    ...
    def update(self, *args):
        ...
        if self.lifespan <= 0:
            self.kill()
            return
        alpha = 255 * (self.lifespan / self.max_lifespan)
...
# --- CORRECTED: Re-added draw_text_center function ---
...
# --- Main Game Loop (Reverted from the complex main_app structure) ---
def main():
    running = True
    game_over = False
    ...
    shockwave_particles = pygame.sprite.Group()
    ...
    while running:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.button == 3 and not game_over:
                    new_particles = player.activate_shockwave()
                    if new_particles:
                        all_sprites.add(new_particles)
                        shockwave_particles.add(new_particles)
        ...
        if not game_over:
            ...
            shockwave_hits = pygame.sprite.groupcollide(asteroids, shockwave_particles, False, True)
            for hit_asteroid in shockwave_hits:
            ...
            sw_hits_ships = pygame.sprite.groupcollide(enemy_ships, shockwave_particles, True, True)
        ...
        if cooldown_elapsed < player.shockwave_cooldown:
            ...
            draw_text_center("Shockwave", bar_x + bar_total_width / 2, bar_y + bar_height / 2, font)
        else:
            pygame.draw.rect(screen, UI_READY_COLOR, (bar_x + 1, bar_y + 1, bar_total_width - 2, bar_height - 2))
            draw_text_center("SHOCKWAVE READY", bar_x + bar_total_width / 2, bar_y + bar_height / 2, font)
        ...
        if game_over:
            ...
            draw_text_center("Press 'R' to Restart", SCREEN_WIDTH / 2 - 100, SCREEN_HEIGHT / 2 + 20)
