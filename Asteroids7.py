...
# Adjusted font sizes for better UI
small_font = pygame.font.SysFont('arial', 18) # Smaller font for UI elements
font = pygame.font.SysFont('arial', 24)
large_font = pygame.font.SysFont('arial', 80) # Main game over text remains large
...
def main():
    ...
    while running:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_r and game_over:
                    ...
                    main()
                    return # Exit the current game loop after restarting
        ...
        draw_text(f"Score: {score}", 10, 10, font_obj=font); draw_text(f"Lives: {lives}", SCREEN_WIDTH - 100, 10, font_obj=font)
        ...
        # --- CORRECTED Shockwave UI ---
        cooldown_elapsed = now - player.last_shockwave_time
        shockwave_text_color = WHITE
        if cooldown_elapsed >= player.shockwave_cooldown:
            shockwave_text = "Shockwave: Ready"
            shockwave_text_color = UI_READY_COLOR # Green when ready
        else:
            shockwave_text = "Shockwave: Cooldown"
            # Optional: You could make cooldown text red or another color if desired
            # shockwave_text_color = UI_COOLDOWN_COLOR
        
        # Position the text at the bottom right
        shockwave_text_surface = small_font.render(shockwave_text, True, shockwave_text_color)
        shockwave_text_rect = shockwave_text_surface.get_rect(bottomright=(SCREEN_WIDTH - 10, SCREEN_HEIGHT - 10))
        screen.blit(shockwave_text_surface, shockwave_text_rect)
        # --- END CORRECTION ---
